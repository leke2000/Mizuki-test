你好，我是 **leke**（这里叫「我若心安好」），欢迎来到我的小站 👋

**Java 后端工程师 · 医疗信息化领域** —— 我从医院机房的一线实施走出来，带着真实的生产问题写代码。
...
- **数据同步链路**：SQLServer / Oracle / MySQL / PostgreSQL 多数据源 EMR 数据同步的实施、排障与验证
- **生产环境运维**：Tomcat / 中间件部署、Wireshark 抓包定位接口故障、Windows / Linux 双平台机房值守
...
| 后端 | Spring Boot · SpringMVC · MyBatis · Maven 多模块 |
| 前端 | Vue 3 · 原生 JS 单文件应用 · GitHub Pages 部署 |
| 数据库 | MySQL · SQLServer · Oracle · PostgreSQL |
| 工程化 | Git · GitHub Actions · Docker 部署 · REST API 设计 |
...
- **[实施工程师排障手记](https://leke2000.github.io/blog/)** — 来自真实环境的实操笔记：Arthas 火焰图、tcpdump + Wireshark 抓包、EMR 同步手册、住院回写时序图

## 目前在学习

- Java 并发编程深入（JUC、线程池源码）
- Spring Boot 自动配置原理 + 自研 starter
- 系统设计：高并发数据同步架构（从医院多数据源场景抽象）
...
- **GitHub**：[leke2000](https://github.com/leke2000) —— 欢迎 Star 和 Follow
- **邮箱**：[1793023565@qq.com](mailto:1793023565@qq.com) —— 合作、交流或只是打个招呼都可以

## 写在最后

- 🔧 **生产 sense**：先做了多年部署排障再写代码——所以我造的工具都为真实运维痛点而生
- 🏥 **领域纵深**：医疗信息化不是 CRUD，是多方博弈下的系统落地能力
- 🎯 **在造的东西**：把 EMR 数据同步经验抽象成通用数据校验框架（进行中）
